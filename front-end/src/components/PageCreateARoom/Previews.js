...
function Previews(props) {
  const [files, setFiles] = useState(props.hinhAnh);
  const {getRootProps, getInputProps} = useDropzone({
    ...
    onDrop: acceptedFiles => {
      ...
      props.upDate(acceptedFiles.map(file => Object.assign(file, {
        preview: URL.createObjectURL(file)
      })));
    }
  });
  ...
  return (
    <section className="container" style={{marginTop:'5rem',maxWidth:'80rem'}}>
      ...
      <div {...getRootProps({className: 'dropzone'})}>
        <input {...getInputProps()} />
        <p>Kéo và thả hình tại đây hoặc click vào để đăng tải ảnh</p>
      </div>
