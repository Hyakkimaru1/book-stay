  ...
  return data === null ? null : (
    <div className="Home row">
      <section className="Home__background">
        <img className="Home__background--img" src={BackgroundHome} alt="">
        </img>
      </section>
      <section className="Home__topplaces">
        ...
        <p className="Home__topplaces--detail">
          Các địa điểm thú vị đang chờ đón bạn
        </p>
        <div style={{ position: "relative" }}>
          <div
            className="Home__button Home__button--right"
            id="buttonplaces_right"
            onClick={() => {
              if (item < 2) {
                setItem(item + 1);
                if (item + 1 == 2) {
                  document.getElementById("buttonplaces_right").style.opacity =
                    0;
                }
                document.getElementById("slide__places").style.transform =
                  `translateX(-${(item + 1) * 230.8}px)`;
              }
              if (
          ...
          <div style={{ overflow: "hidden" }}>
            <div className="Home__topplaces--card" id="slide__places">
              ...
              <CardPlace
                ...
                name="Đà lạt"
                onChange={(item) => handleChange(item)}
                places={data.rooms.dl}
              />
              <CardPlace
                img="https://cdn.luxstay.com/home/location/location_6_1559786202.png"
                name="Hội An"
                onChange={(item) => handleChange(item)}
                places={data.rooms.ha}
              />
            </div>
          </div>
          <div
            className="Home__button Home__button--left"
            id="buttonplaces_left"
            onClick={() => {
              if (item > 0) {
                setItem(item - 1);
                if (item - 1 == 0) {
                  document.getElementById("buttonplaces_left").style.opacity =
                    0;
                }
                document.getElementById("slide__places").style.transform =
                  `translateX(-${(item - 1) * 230.8}px)`;
        ...
        <div style={{ width: "100%" }} className="Home__topplaces--card">
          {data.mostBook.map((ele) =>
            <CardDetail
              key={ele.id}
              id={ele.id}
              rates={ele.rates && ele.rates}
              ten={ele.ten}
              img={ele.img && ele.img.img}
              soKhachToiDa={ele.soKhachToiDa}
              soPhong={ele.soPhong}
              soGiuong={ele.soGiuong}
              giaNgayThuong={ele.giaNgayThuong}
              star={ele.star && ele.star}
            />
          )}
        </div>
      </section>
